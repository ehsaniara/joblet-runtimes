#!/bin/bash
# Self-contained Amazon Linux ARM64 GraalVM JDK 21 Runtime Setup
...
install_graalvm() {
    ...
    wget -O graalvm.tar.gz "$GRAALVM_URL" || curl -L -o graalvm.tar.gz "$GRAALVM_URL"
...
install_dependencies() {
    ...
    # Install essential packages using yum
    yum install -y \
        wget \
        ca-certificates \
        glibc \
...
copy_system_files() {
    ...
    # Copy standard C library and related files (ARM64 specific)
...
description: "GraalVM Community Edition JDK 21 Runtime (Amazon Linux ARM64)"
...
mounts:
  ...
  - source: "isolated/usr/sbin"
    target: "/usr/sbin"
    readonly: true
  # Essential libraries (Amazon Linux uses lib64)
  - source: "isolated/lib"
    target: "/lib"
    readonly: true
...
environment:
  ...
  LD_LIBRARY_PATH: "/usr/lib64:/lib64:/usr/lib/jvm/graalvm-21/lib"
EOF
...
main() {
    echo ""
    echo "🚀 Starting GraalVM JDK 21 Runtime Installation (Amazon Linux ARM64)"
    echo "=================================================================="
    ...
    echo ""
    echo "🎉 GraalVM JDK 21 Runtime Installation Complete! (Amazon Linux ARM64)"
    echo "===================================================================="
