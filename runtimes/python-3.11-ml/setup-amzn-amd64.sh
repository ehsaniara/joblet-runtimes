...
create_directories() {
    ...
    local dirs=(
        ...
        opt/venv etc tmp proc usr/lib64
    )
...
copy_system_files() {
    ...
    # Essential libraries (Amazon Linux paths)
...
install_python() {
    ...
    # Copy Python runtime - Amazon Linux often has Python in /usr/lib64
    if [ -d "/usr/lib64/python3.11" ]; then
        ...
        echo "Copying Python 3.9 (Amazon Linux 2 default)..."
        cp -r "/usr/lib/python3.9" "$ISOLATED_DIR/usr/lib/" 2>/dev/null || true
    elif [ -d "/usr/lib/python3.7" ]; then
        echo "Copying Python 3.7 (Amazon Linux 2 legacy)..."
        cp -r "/usr/lib/python3.7" "$ISOLATED_DIR/usr/lib/" 2>/dev/null || true
    fi
    ...
    [ -f python3.7 ] && [ ! -f python ] && ln -sf python3.7 python 2>/dev/null || true
...
copy_packages_from_host() {
    ...
    # Search paths where ML packages might be installed (Amazon Linux specific)
