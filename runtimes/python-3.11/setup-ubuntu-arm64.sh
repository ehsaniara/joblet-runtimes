#!/bin/bash
# Basic Python 3 Runtime Setup for Ubuntu/Debian ARM64
...
for lib_dir in /lib64 /usr/lib64 /lib /usr/lib /lib/aarch64-linux-gnu /usr/lib/aarch64-linux-gnu; do
    if [ -d "$lib_dir" ]; then
...
[ -f "/lib/aarch64-linux-gnu/ld-linux-aarch64.so.1" ] && {
    mkdir -p "$ISOLATED_DIR/lib/aarch64-linux-gnu"
    cp -P "/lib/aarch64-linux-gnu/ld-linux-aarch64.so.1" "$ISOLATED_DIR/lib/aarch64-linux-gnu/" 2>/dev/null || true
}
...
# Find and copy Python libraries (preserve original version path)
for lib_path in "/usr/lib/python$python_version" "/usr/local/lib/python$python_version"; do
    if [ -d "$lib_path" ]; then
        echo "Copying Python library from $lib_path..."
        mkdir -p "$ISOLATED_DIR/usr/lib/python$python_version"
        cp -r "$lib_path"/* "$ISOLATED_DIR/usr/lib/python$python_version/" 2>/dev/null || true

        # Also create a symlink for python3.11 compatibility
        if [ "$python_version" != "3.11" ]; then
            mkdir -p "$ISOLATED_DIR/usr/lib/python3.11"
            ln -sf "../python$python_version"/* "$ISOLATED_DIR/usr/lib/python3.11/" 2>/dev/null || true
        fi
        break
    fi
done
...
# Runtime configuration with ARM64 library paths
