#!/bin/bash
# Self-contained RedHat Enterprise Linux ARM64 OpenJDK 21 Runtime Setup
...
install_java_packages() {
    echo "Installing OpenJDK packages via YUM (RHEL ARM64)..."
...
copy_system_libraries() {
    echo "Copying essential system libraries (RedHat ARM64)..."
    
    # Copy essential libraries for Java runtime (ARM64 specific)
    local lib_dirs=("/usr/lib64" "/lib64" "/usr/lib/aarch64-linux-gnu" "/lib/aarch64-linux-gnu")
