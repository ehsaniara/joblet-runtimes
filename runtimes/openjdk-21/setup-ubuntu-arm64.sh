#!/bin/bash
# Self-contained Ubuntu/Debian ARM64 OpenJDK 21 Runtime Setup
...
setup_runtime_directories() {
    ...
    mkdir -p isolated/usr/lib/aarch64-linux-gnu  # ARM64 specific
    mkdir -p isolated/lib/aarch64-linux-gnu      # ARM64 specific
...
find_java_installation() {
    ...
    # ARM64 specific Java installation paths
    local java_homes=(
        "/usr/lib/jvm/java-21-openjdk-arm64"
        "/usr/lib/jvm/java-21-openjdk-aarch64"
        "/usr/lib/jvm/java-21-openjdk"
        "/usr/lib/jvm/java-11-openjdk-arm64"
        "/usr/lib/jvm/java-11-openjdk-aarch64"
...
install_java_packages() {
    echo "Installing OpenJDK packages via APT (ARM64)..."
    ...
    echo "Installing OpenJDK 21 packages for ARM64..."
...
copy_system_libraries() {
    echo "Copying essential system libraries (ARM64)..."
    
    # Copy essential libraries for Java runtime - ARM64 specific paths
    local lib_dirs=("/usr/lib/aarch64-linux-gnu" "/lib/aarch64-linux-gnu" "/lib64")
    
    for lib_dir in "${lib_dirs[@]}"; do
        if [ -d "$lib_dir" ]; then
            echo "Copying libraries from $lib_dir..."
            local lib_patterns=("libc.so*" "libdl.so*" "libpthread.so*" "librt.so*" "libm.so*" "libz.so*" "libgcc_s.so*" "ld-linux-aarch64.so*" "libstdc++.so*")
    ...
    # Copy ARM64 dynamic linker
    if [ -f /lib/ld-linux-aarch64.so.1 ]; then
        cp /lib/ld-linux-aarch64.so.1 isolated/lib/
